...
def predictive_score_metrics_multi(ori_data, generated_data):
  """Report the performance of Post-hoc RNN one-step ahead prediction.
  ...
  # Basic Parameters
  no, seq_len, dim, dd = np.asarray(ori_data).shape
  ...
  for num in range(dd):
    ...
    for itt in range(iterations):
      ...
      # X_mb = list(generated_data[i][:-1,:(dim-1)] for i in train_idx)
      X_mb = list(generated_data[i][:-1, :(dim), num] for i in train_idx)
      T_mb = list(generated_time[i] - 1 for i in train_idx)
      Y_mb = list(
        np.reshape(generated_data[i][1:, (dim - 1), num], [len(generated_data[i][1:, (dim - 1), num]), 1]) for i in train_idx)
    ...
    # X_mb = list(ori_data[i][:-1,:(dim-1)] for i in train_idx)
    X_mb = list(ori_data[i][:-1, :(dim), num] for i in train_idx)
    T_mb = list(ori_time[i] - 1 for i in train_idx)
    Y_mb = list(np.reshape(ori_data[i][1:, (dim - 1), num], [len(ori_data[i][1:, (dim - 1), num]), 1]) for i in train_idx)
    ...
    predictive_score = MAE_temp / no
    predi += predictive_score

    tf.reset_default_graph()

  predictive_score = predi / dd
  return predictive_score
